...
- **19 new tests** in `src/sync/scheduler.test.ts`: `pickDueCommands` truth table (ticks 0 / 1 / 4 / 7 / 8 / 24), start/stop guardrails (disabled / no token / empty allowlist / idempotent start / stop clears), tick behavior (HIGH-only on tick 1, HIGH+MEDIUM on tick 4, rate-limit skip below threshold, no skip when snapshot null, `lastBackgroundRunAt` recording, throw isolation between commands, failure aggregation without abort). Total: **508 tests across 28 suites**.
